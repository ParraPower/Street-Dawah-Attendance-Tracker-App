...
export class GetEmirSessionPreferencesByUserUseCase {
  constructor(private readonly repo: IEmirSessionPreferenceRepository) {}
  async execute(userId: number): Promise<EmirSessionPreferenceDto[]> {
    return (await this.repo.findByUserId(userId)).map((item) => mapper.map(item, EmirSessionPreferenceEntity, EmirSessionPreferenceDto));
  }
}
